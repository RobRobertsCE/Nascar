...
namespace Tweetinvi.Core.Interfaces.Async
{
    public interface IUserAsync
    {
        ...
        // Friendship
        Task<IRelationship> GetRelationshipWithAsync(IUser user);

        // Timeline
        Task<IEnumerable<ITweet>> GetUserTimelineAsync(int maximumTweet = 40);
        Task<IEnumerable<ITweet>> GetUserTimelineAsync(IUserTimelineRequestParameters timelineRequestParameters);
